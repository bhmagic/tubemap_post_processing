function making_volume_csv(mat_file_after_converting, dir_root_OG )
...
% label_OG = niftiread(label_file_name);
% label_OG = tiffreadVolume(label_file_name);
...
% 
% location_1 = e_edge_geometry_indices(:,1)+1;
% location_2 = e_edge_geometry_indices(:,2);
% 
% g_edge_v2_location = {};
% g_edge_v2_length = {};
% g_edge_v2_radii = {};
% 
% 
% parfor ii = 1:length(location_1)
% %     ii =1
%     g_edge_v2_radii{ii} = g_edge_geometry_radii(location_1(ii)+1: location_2(ii)-1);
%     g_edge_v2_coordinates{ii} = g_edge_geometry_coordinates(location_1(ii)+1: location_2(ii)-1,:);
%     temppp = g_edge_geometry_coordinates(location_1(ii): location_2(ii)-2,:) - g_edge_geometry_coordinates(location_1(ii)+2: location_2(ii),:);
%     
%     g_edge_v2_length{ii} = sqrt(sum(temppp.*temppp, 2))./2;
%     
% end
...
finnal_table = cell2table([num2cell(ROI_table.id), ...
                        ...
                        num2cell(total_volume_fin'), ...
                        num2cell(net_down_sample_l_fin'), ...
                        num2cell(net_down_sample_l_v_fin'), ...                        
